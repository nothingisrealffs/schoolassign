...
def parse_mech_data(file_content):
    ...
    patterns = {
        ...
        'Weapons': {
            'weapon_count': r'Weapons:\d+',
            'weapon_list': r'((?:.+,\s*.+\n?)+)'  # Captures the list of weapons
        },
        'Quirks': {
            'quirk': r'quirk:\s*(.+)'
        },
        ...
        if isinstance(fields, dict):
            for field, pattern in fields.items():
                if isinstance(pattern, dict):
                    ...
                    if match:
                        if field == 'weapon_list':
                            # Split the captured weapon list into individual entries
                            weapon_entries = match.group(1).strip().splitlines()
                            table_data[field] = [weapon.strip() for weapon in weapon_entries]
                        else:
