...
def generate_sql(file_content,output_filename):
    ...
    for i, line in enumerate(lines):
        ...
        # History/Overview/Capabilities
            ...
            history_overview_data['overview'] = extract_value(line)
        elif line.startswith("capabilities:"):
            history_overview_data['capabilities'] = extract_value(line)
        elif line.startswith("deployment:"):
            history_overview_data['deployment'] = extract_value(line)
        elif line.startswith("history:"):
            history_overview_data['history'] = extract_value(line)
    ...
    # Chassis table
    sql_statements.append(f"INSERT INTO Chassis (chassis_name, model, mul_id, mass, role, config, techbase, era, source, rules_level) "
                          f"VALUES ('{chassis_data['chassis_name']}', '{chassis_data['model']}', {chassis_data['mul_id']}, '{config_data['mass']}, " 
                          f"'{chassis_data['role']}', '{chassis_data['config']}', '{chassis_data['techbase']}', "
                          f"{chassis_data['era']}, '{chassis_data['source']}', {chassis_data['rules_level']});")

    # Configuration table
    sql_statements.append(f"INSERT INTO Configuration (chassis_id, engine, structure, myomer, heat_sinks, walk_mp, jump_mp) "
                          f"VALUES ((SELECT chassis_id FROM Chassis WHERE chassis_name='{chassis_data['chassis_name']}'), "
                          f"'{config_data['engine']}', '{config_data['structure']}', '{config_data['myomer']}', "
                          f"'{config_data['heat_sinks']}', {config_data['walk_mp']}, {config_data['jump_mp']});")
    ...
    for weapon in weapons_data:
        sql_statements.append(f"INSERT INTO Weapons (chassis_id, weapon_name, location) "
                              f"VALUES ((SELECT chassis_id FROM Chassis WHERE chassis_name='{chassis_data['chassis_name']}'), "
                              f"'{weapon['weapon_name']}', '{weapon['location']}');")
    ...
    for component in actuators_components_data:
        sql_statements.append(f"INSERT INTO Actuators_Components (chassis_id, location, component_name) "
                              f"VALUES ((SELECT chassis_id FROM Chassis WHERE chassis_name='{chassis_data['chassis_name']}'), "
                              f"'{component['location']}', '{component['component_name']}');")
    ...
    for manufacturer in manufacturer_data:
        sql_statements.append(f"INSERT INTO Manufacturer (chassis_id, manufacturer_name, primary_factory) "
                              f"VALUES ((SELECT chassis_id FROM Chassis WHERE chassis_name='{chassis_data['chassis_name']}'), "
                              f"'{manufacturer['manufacturer_name']}', '{manufacturer['primary_factory']}');")
        for system in manufacturer['systems']:
            sql_statements.append(f"INSERT INTO SystemManufacturer (manufacturer_id, system) "
                                  f"VALUES ((SELECT manufacturer_id FROM Manufacturer WHERE manufacturer_name='{manufacturer['manufacturer_name']}'), "
                                  f"'{system}');")
    ...
    #                      f"VALUES ((SELECT chassis_id FROM Chassis WHERE chassis_name='{chassis_data['chassis_name']}'), "
    #                      f"'{history_overview_data['overview']}', '{history_overview_data['capabilities']}', "
    #                      f"'{history_overview_data['deployment']}', '{history_overview_data['history']}');")
        # History and Overview table
    sql_statements.append(f"INSERT INTO History_Overview (chassis_id, overview, capabilities, deployment, history) "
...
process_mtf_files(folder)
    # Write SQL to file
#    with open("battlemech_albatross.sql", "w") as sql_file:
#        sql_file.write("\n".join(sql_statements))

# Example usage
#if __name__ == "__main__":
#    with open("./GitHub/schoolassign/2024_school_year/db_prog/app/Albatross ALB-3U.mtf", "r") as f:
#        file_content = f.read()
#    generate_sql(file_content)
